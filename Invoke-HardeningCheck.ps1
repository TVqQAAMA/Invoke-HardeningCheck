Function Invoke-HardeningCheck {
    ...
    .SYNOPSIS
        ...
        Invoke-HardeningCheck - Checks and hardens your Windows configuration
    ...
    .DESCRIPTION
        ...
        The configuration of the system is retrieved and assessed using a finding csv list. It reading settings from the registry and uses other modules to read configurations outside the registry. 


    .PARAMETER FileFindingList

        Path to a finding list in CSV format. HardeningCheck has one list each for machine and user settings.
    ...
    .PARAMETER Log
        ...
        name is assigned by HardeningCheck itself and the file is stored in the same directory as the script.
    ...
    .PARAMETER Report
        ...
        The file name is assigned by HardeningCheck itself and the file is stored in the same directory as the script.
    ...
    .EXAMPLE
        ...
        Description: HardeningCheck performs an audit, saves the results and creates a log file:
        Invoke-HardeningCheck -Mode Audit -Log -Report

        Description: HardeningCheck performs an audit with a specific list and does not show machine information:
        Invoke-HardeningCheck -FileFindingList .\lists\finding_list_0x6d69636b_user.csv -SkipMachineInformation

        Description: HardeningCheck ready only the setting with the default list, and saves the results in a specific file:
        Invoke-HardeningCheck -Mode Config -Report -Report C:\tmp\my_HardeningCheck_report.log
    ...
    If ($Log.IsPresent -and $LogFile.Length -eq 0) {
        $LogFile = "Log_"+$Hostname+"_"+$ListName+"-$FileDate.log"
    }
    If ($Report.IsPresent -and $ReportFile.Length -eq 0) {
        $ReportFile = "Report_"+$Hostname+"_"+$ListName+"-$FileDate.csv"
    }
    If ($Report.IsPresent) {
        $Message = '"ID","Name","Severity","Result","Recommended","Risk"'
    ...
    # Header
    #
    Write-ProtocolEntry -Text "Starting HardeningCheck" -LogLevel "Info"
    ...
    If ($Mode -eq "Audit" -or $Mode -eq "Config") {
        ...
        # A CSV finding list is imported. HardeningCheck has one machine and one user list.
        ...
        ForEach ($Finding in $FindingList) {
            ...
            # to a file, which means HardeningCheck must create a temporary file
            # and afterwards delete it. HardeningCheck is very orderly.            
            #
            ...
            If ($Mode -eq "Audit") {
                ...
                If ($ResultPassed) {
                    ...
                    If ($Report) {
                        $Message = '"'+$Finding.ID+'","'+$Finding.Name+'","'+$Finding.Severity+'","'+$Result+'","'+$Finding.RecommendedValue+'","'+$Finding.Rationale+'"'
                        Add-ResultEntry -Text $Message
                    }
            ...
            #
                ...
                If ($Report) {
                    $Message = '"'+$Finding.ID+'","'+$Finding.Name+'",,"'+$Result+'","'+$Finding.Rationale+'"'
                    Add-ResultEntry -Text $Message
                }
            }
        }
    ...
    If ($Mode -eq "Audit") {
        ...
        If ($StatsError -gt 0) {
            Write-ProtocolEntry -Text "Errors occurred due to missing admin rights or tools. For a complete result, these errors should be resolved. Total errors: $StatsError" -LogLevel "Error"
        }
        Write-Output "`n"    
        Write-ProtocolEntry -Text "Statistics: Total checks: $StatsTotal - Passed: $StatsPassed, Low: $StatsLow, Medium: $StatsMedium, High: $StatsHigh." -LogLevel "Info"
    }
    Write-Output "`n"
}